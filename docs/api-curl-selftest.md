...
```bash
LOGIN=$(curl -s -X POST "$HOST/api/auth/wechat/login" \
...
```bash
curl -s -X PUT "$HOST/api/users/me" \
...
```bash
curl -s "$HOST/api/dna/questions" | jq
```

### 3.4 提交 DNA

```bash
curl -s -X POST "$HOST/api/dna/submissions" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "answers":[
...
```bash
curl -s -X POST "$HOST/api/users/location" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "$HOST/api/match/recommendations" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
curl -s "$HOST/api/discovery/groups" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
curl -s "$HOST/api/verification/me/status" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
curl -s -X POST "$HOST/api/verification/wechat" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
CREATE_GROUP=$(curl -s -X POST "$HOST/api/groups" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "$HOST/api/groups/my?type=created" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
curl -s "$HOST/api/groups/$GROUP_ID" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
START_VOTE=$(curl -s -X POST "$HOST/api/groups/$GROUP_ID/vote/start" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"duration_hours":24}')
...
```bash
CURRENT=$(curl -s "$HOST/api/groups/$GROUP_ID/votes/sessions/current" \
  -H "Authorization: Bearer $TOKEN")
...
```bash
curl -s -X POST "$HOST/api/groups/$GROUP_ID/votes" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "$HOST/api/groups/$GROUP_ID/votes/result" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
CREATE_ORDER=$(curl -s -X POST "$HOST/api/orders" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"group_id\":$GROUP_ID}")
...
```bash
PAY=$(curl -s -X POST "$HOST/api/orders/$ORDER_ID/pay/wechat" \
  -H "Authorization: Bearer $TOKEN")
...
```bash
curl -s -X POST "$HOST/api/payments/wechat/notify" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "$HOST/api/orders/$ORDER_ID" \
  -H "Authorization: Bearer $TOKEN" | jq

curl -s "$HOST/api/orders" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
curl -s -X POST "$HOST/api/orders/$ORDER_ID/refund/apply" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
```bash
TICKET=$(curl -s -X POST "$HOST/api/tickets/generate" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"order_id\":$ORDER_ID}")
...
```bash
curl -s -X POST "$HOST/api/tickets/verify" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -s -X POST "$HOST/api/ratings" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
curl -s "$HOST/api/groups/$GROUP_ID/ratings/summary" \
  -H "Authorization: Bearer $TOKEN" | jq
```
...
- `404 Not Found`：路径不对，确认是否使用 `/api` 前缀。
